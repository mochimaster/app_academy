...
def andrews_films_and_leads
  ...
  JOIN castings ON castings.movie_id = movies.id
  JOIN actors ON actors.id = castings.actor_id 
  JOIN actors a2 ON a2.id = castings.actor_id
  WHERE castings.ord = 1  
  AND title IN--(SUBQUERY THAT RETURNS all movies that Julie andrews is in)
  (
    SELECT
     title
    FROM
  ...
  -- GROUP BY 
  --   movies.title, actors.name, actors.id
  -- HAVING 
  --   actors.id = (
  --     SELECT
  --      id 
...
def colleagues_of_garfunkel
  ...
  execute(<<-SQL)
    ...
    FROM movies
    FULL OUTER JOIN castings ON movies.id = castings.movie_id
    FULL OUTER JOIN  actors  ON actors.id = castings.actor_id
    WHERE
    movies.title IN 
        (
          SELECT title
